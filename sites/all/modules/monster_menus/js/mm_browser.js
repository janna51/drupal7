...
Drupal.behaviors.mm_browser_init = {
  attach: function (context, settings) {
    ...
    // Work around splitter.js setting absolute width for the right hand frame,
    // which causes the scroll bar to disappear.
    $('#mmtree-browse-items').resize(function() {
    ...
    $.jstree._themes = settings.basePath + settings.libraryPath + "/jsTree/themes/";
    ...
    var startPath = settings.startBrowserPath;
    if (document.cookie.indexOf("goto_last=1") >= 0) {
      ...
      startPath = settings.lastBrowserPath;
    }
...
Drupal.mm_browser_init_jstree = function (path) {
  var setHeight = function() {
    ...
    if (ht > 0) $("#mmtree-browse-browser,#mmtree-browse-tree,#mmtree-browse-iframe,#mmtree-browse-items").height(ht + 5);
  };
...
Drupal.mm_browser_params_json = function() {
  ...
  for (i in Drupal.settings) {
    if (i.substring(0, 7) == 'browser' && i.length > 7)
      obj[i] = encodeURI(Drupal.settings[i]);
  }
  return obj;
}
...
Drupal.mm_browser_refresh_right = function(node) {
  ...
  params = Drupal.mm_browser_params_json();
  params.id = node.id;
  $.getJSON(Drupal.settings.basePath + 'mm-browser-getright',
    params,
    function(data) {
      $('#mmtree-assist-title').html(data.title);
      $('#mmtree-assist-links').html(data.links);
      $('#mmtree-assist-content').html(data.body);
      if (data.lastviewed) Drupal.settings.lastBrowserPath = data.lastviewed;
    }
  );
};
...
Drupal.mm_browser_gallery_add = function(mmtid, filename, nid, fid) {
  parent && parent.mmListInstance && parent.mmListInstance.addFromChild($("#mmbr-" + mmtid)[0], 0, nid && fid ? nid + '/' + fid : '', filename);
  return false;
};
